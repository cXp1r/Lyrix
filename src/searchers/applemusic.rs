...
impl ISearcher for ApplemusicSearcher {
    async fn search_for_results_by_string(
        ...
        let res = resp.results.ok_or_else(|| SearcherError::NoResults {
            label: self.label().to_string(),
            query: search_string.to_string(),
        })?;
        let songs = res.songs.ok_or_else(|| SearcherError::NoResults {
            label: self.label().to_string(),
            query: search_string.to_string(),
        })?;
        let songsv = songs.data.ok_or_else(|| SearcherError::NoResults {
            label: self.label().to_string(),
            query: search_string.to_string(),
        })?;
        for song in songsv {
            let id = song.id.clone().unwrap_or_default();
            let info = song.attributes.ok_or_else(|| SearcherError::MissingField {
                label: self.label().to_string(),
            ...
            let duration = info.duration_in_millis.map(|d| d as u32);
            let has_lyrics = info.has_lyrics.unwrap_or(false);
